  ...
  return (
    <div className="rounded-2xl bg-slate-900 border border-slate-800 shadow-2xl overflow-hidden flex flex-col">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <div className="relative min-w-[240px] flex-1 md:flex-initial">
              ...
              <input
                ...
                className="w-full pl-9 pr-3 py-1.5 bg-slate-950 border border-slate-800 rounded-xl text-xs text-white placeholder-slate-500 focus:outline-none focus:ring-1 focus:ring-emerald-500 focus:border-emerald-500 transition font-mono"
              />
              {searchQuery && (
                <button
            ...
            <select
              ...
              className="px-2.5 py-1.5 bg-slate-950 border border-slate-800 rounded-xl text-xs text-slate-200 focus:outline-none focus:ring-1 focus:ring-emerald-500 cursor-pointer"
            >
            ...
            {(searchQuery || selectedLga || selectedCrop) && (
              <button
                ...
                className="p-1.5 rounded-lg bg-slate-800 hover:bg-slate-700 text-slate-400 hover:text-white border border-slate-700 text-xs flex items-center space-x-1 transition"
              >
      ...
      <div className="overflow-x-auto min-h-[300px] relative">
        {loading && (
          <div className="absolute inset-0 bg-slate-950/60 backdrop-blur-[2px] z-20 flex items-center justify-center">
            <div className="flex items-center space-x-2 px-4 py-2 rounded-xl bg-slate-900 border border-slate-800 shadow-xl">
              <div className="animate-spin rounded-full h-4 w-4 border-2 border-emerald-400 border-t-transparent"></div>
              <span className="text-xs font-semibold text-slate-300">Loading Registry Records...</span>
            </div>
          </div>
        )}
              ...
              <tr>
                <td colSpan={9} className="py-12 text-center text-slate-500">
                  <div className="max-w-xs mx-auto space-y-2">
                    ...
                    <p className="font-semibold text-slate-400">No matching farmer records found</p>
              ...
              records.map((farmer) => {
                ...
                return (
                  ...
                  >
                    ...
                    <td className="py-3 px-4">
                      <div className="font-mono text-emerald-400 tracking-wider bg-slate-950/80 px-2 py-0.5 rounded border border-slate-800 inline-block">
                        {farmer.nin}
                      </div>
                    </td>
                    ...
                    <td className="py-3 px-4">
                      <div className="font-mono text-teal-300 tracking-wider bg-slate-950/80 px-2 py-0.5 rounded border border-slate-800 inline-block">
                        {farmer.bvn}
                      </div>
                    </td>
                    ...
                    {/* LGA & Ward */}
                    <td className="py-3 px-4">
                      <div className="font-semibold text-slate-200">{farmer.lga}</div>
                    ...
                    <td className="py-3 px-4 text-right">
                      <button
                        ...
                        className="inline-flex items-center space-x-1 px-2.5 py-1 rounded-lg bg-slate-800 hover:bg-emerald-500 hover:text-slate-950 text-slate-300 text-xs font-semibold border border-slate-700 transition"
                      >
      ...
      <div className="p-3.5 bg-slate-950/90 border-t border-slate-800 flex items-center justify-between text-xs text-slate-400">
        <div>
          Showing <span className="font-semibold text-white">{rangeStart}</span> to{' '}
          <span className="font-semibold text-white">{rangeEnd}</span> of{' '}
          <span className="font-semibold text-white">{totalCount.toLocaleString()}</span> entries
        </div>
